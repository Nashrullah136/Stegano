...
class lz77:
	...
	def encode(self,text_name,sbSize,labSize):
		...
		wrt=open("encoded.txt","w");wrt.write(finalstr);wrt.close()
...
class deflate:
	...
	def encode(self,file,sb,lab):
		...
		wrt_ky=open("key_o.txt","w");wrt_ky.write(key_o);wrt_ky.close()
		wrt_ky=open("key_s.txt","w");wrt_ky.write(key_s);wrt_ky.close()

		wrt_final=open("deflated.txt","w");wrt_final.write(text1+"\n"+text2)

		return text1, text2, key_o, key_s
	...
	def huffman_decode(self,deflated,key_o,key_s):
		...
		wrtt=open("unhuffed.txt","w");wrtt.write(finalstr);wrtt.close()
		return finalstr
	...
	def decode(self,file,key_o,key_s): 
		...
		deflated=file
		keyo=key_o ; keys = key_s
		self.file_name=file

		huffman_decode = self.huffman_decode(deflated,keyo,keys)
		final_decoded=lz77().decode(huffman_decode)
